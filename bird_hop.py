...
pygame.init()
# Program Settings / Fonts / Sounds / Game Mechanics Configurations
score = 0
previous_score = 0
show = 0
...
# Animations & Game Settings | The main function calls these functions in every frame.
...
# Custom Events: Create custom events, which then the main function will check.
...
while True:
    # MAIN FUNCTION: If the game is still active, we check for any user inputs, event timers, collisions,
    # and the program responds with position changes, increment of points.
    for event in pygame.event.get():
        ...
        if game_active:
            ...
            if (event.type == pygame.KEYDOWN) and (event.key == pygame.K_SPACE):
                ...
                if bird_rect.bottom >= 300:
                    bird_gravity = -20
            if (event.type == pygame.MOUSEBUTTONDOWN) and (bird_rect.collidepoint(event.pos)) and (
            ...
            if event.type == false_game_timer:
                if randint(0, 2) == 1:
                    ...
                    show = 0
                    continue
    # If the game is active, it displays the mini scoreboard,
    # and changes the position/animations.
    # this also checks for any game settings/events, and responds promptly
    # If the game is not active, we change the screen to the score screen, and wait for a user's input
    if game_active == True:
        score = display_score()
